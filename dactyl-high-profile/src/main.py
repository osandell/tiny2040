...
def thumb_wall_simple(place_fn1, idx1, place_fn2, idx2, **kwargs):
    return wall_connect_from_placer(place_fn1, idx1, place_fn2, idx2, **kwargs)


def thumb_switches_top2():
    return union(
        thumb_r_placer(single_switch),
        thumb_m_placer(single_switch),
    )


def thumb_walls_top2():
    return union(
        # top: m -> r
        thumb_wall_simple(thumb_m_placer, square_idx_tl, thumb_m_placer, square_idx_tr),
        thumb_wall_simple(thumb_m_placer, square_idx_tr, thumb_r_placer, square_idx_tl),
        thumb_wall_simple(thumb_r_placer, square_idx_tl, thumb_r_placer, square_idx_tr),
        # right of r
        thumb_wall_simple(thumb_r_placer, square_idx_tr, thumb_r_placer, square_idx_br),
        # bottom: r -> m
        thumb_wall_simple(thumb_r_placer, square_idx_br, thumb_r_placer, square_idx_bl),
        thumb_wall_simple(thumb_r_placer, square_idx_bl, thumb_m_placer, square_idx_br),
        thumb_wall_simple(thumb_m_placer, square_idx_br, thumb_m_placer, square_idx_bl),
        # left of m
        thumb_wall_simple(thumb_m_placer, square_idx_bl, thumb_m_placer, square_idx_tl),
    )


def thumb_connectors_top2():
    return hull(
        thumb_m_placer(web_post_tr),
        thumb_m_placer(web_post_br),
        thumb_r_placer(web_post_tl),
...
def thumb_corner_top2():
    ...
    return difference(
        union(
            thumb_switches_top2(),
            thumb_walls_top2(),
            thumb_connectors_top2(),
        ),
        union(blocker(),),
    )


def left_thumb_corner_top2():
    return flip_lr()(thumb_corner_top2())


def write_test():
    render_to_file(thumb_corner_top2(), "things/thumb_corner_top2_right.scad")
    render_to_file(left_thumb_corner_top2(), "things/thumb_corner_top2_left.scad")
